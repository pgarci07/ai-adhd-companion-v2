...
def read_seed_rows(csv_path: Path) -> list[SeedRow]:
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as csv_file:
        ...
        # Older test datasets shipped with a misspelled "decription" header.
        # Accept both spellings so the loader can consume the existing files
        # while also supporting the corrected "description" column going
        # forward. The column remains optional: when absent or blank, the
        # loader preserves the previous fallback to the task title.
        description_field_name = None
        available_fields = set(reader.fieldnames or [])
        if "description" in available_fields:
            description_field_name = "description"
        elif "decription" in available_fields:
            description_field_name = "decription"
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            rows.append(
                SeedRow(
                    local_id=local_id,
                    title=title,
                    description=clean_optional_text(row.get(description_field_name or "")),
...
def get_row_instance_count(row: SeedRow, rrule_instances: int | None) -> int:
    """Return how many instances to create for one root task row."""

    if rrule_instances is not None and row.rrule:
        return rrule_instances
    return row.number_of_instances


def build_root_occurrences(
    row: SeedRow,
    *,
    rrule_instances: int | None = None,
) -> list[tuple[int, datetime, datetime]]:
    """Build root-task instance windows from the first CSV window and RRULE.

    Root tasks are either compound parents or standalone simple tasks. Subtasks
    do not use their own RRULE; they are instantiated under each generated
    parent instance.
    """

    number_of_instances = get_row_instance_count(row, rrule_instances)
    if number_of_instances < 1:
        raise ValueError(
            f"CSV row {row.row_number}: number_of_instances must be at least 1."
        )
    if row.due_at < row.start_at:
        raise ValueError(f"CSV row {row.row_number}: due_date is before start_date.")
    if number_of_instances > 1 and not row.rrule:
        raise ValueError(
            f"CSV row {row.row_number}: rrule is required when number_of_instances > 1."
        )

    duration = row.due_at - row.start_at
    starts = [row.start_at]

    if number_of_instances > 1:
        cursor = row.start_at
        while len(starts) < number_of_instances:
            next_start = next_rrule_start_after(row.rrule or "", row.start_at, cursor)
...
def insert_task(
    ...
    *,
    ...
    payload = {
        "list_id": list_id,
        "title": row.title,
        # The dataset may now carry an explicit description column. Keep the
        # old behavior as a fallback so older rows still load identically.
...
def load_test_data(
    ...
    *,
    ...
    simple_rows = [row for row in rows if not row.is_parent and not row.parent_id]
    child_rows = [row for row in rows if not row.is_parent and row.parent_id]
    ...
    for parent in parent_rows:
        ...
        occurrences = build_root_occurrences(parent, rrule_instances=rrule_instances)
    ...
    for simple in simple_rows:
        size_id = resolve_dimension_id(size_options, simple.size_position, "Size")
        consequence_id = resolve_dimension_id(
            consequence_options,
            simple.consequence_position,
            "Consequence",
        )
        friction_id = resolve_dimension_id(friction_options, simple.friction_position, "Friction")
        occurrences = build_root_occurrences(simple, rrule_instances=rrule_instances)

        if dry_run:
            print(f"[dry-run] simple {simple.local_id}: {simple.title} -> {len(occurrences)} instances")
            created_task_ids[simple.local_id] = f"dry-run-task-{simple.local_id}"
            tasks_inserted += 1
            instances_inserted += len(occurrences)
            continue

        simple_task_id = insert_task(
            client,
            list_id=list_id,
            row=simple,
            parent_task_id=None,
        ...
        created_task_ids[simple.local_id] = simple_task_id
        tasks_inserted += 1

        simple_instance_payloads = [
            {
        ...
        instances_inserted += insert_instances(client, simple_instance_payloads)
    ...
    return {
        ...
        "simple_tasks_inserted": len(simple_rows),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override number_of_instances for root tasks with RRULE "
